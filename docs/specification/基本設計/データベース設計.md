...
※ 中心は「calls」「call_billing_units」「point_transactions」「wallets」の4テーブル。
...
## 6. call_billing_units (1分ごとの課金ユニット)

1分ごとに45 pt消費した記録を確実に残すためのテーブル。
...
- サーバの課金タイマーが2重実行されても unique constraint で二重課金を防げる
- ユーザーの明細画面で「n分でxx pt」など表示できる
- 統計情報 (平均通話時間、LTVなど) も作りやすい

# 7. call_events

通話中に発生したイベントをすべて記録する監査ログテーブルです。

**SFUイベント・課金イベント・WSイベントなど全て記録可能。**
...
1対1通話でも、**参加者情報（JOINED/LEFT）を保存するメリットは大きい** ため作成を推奨します。
...
- billing_unit (1分ごと) + point_transactions によって完全保証
