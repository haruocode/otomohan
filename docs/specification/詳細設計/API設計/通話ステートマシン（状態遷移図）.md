# SFU版 通話ステートマシン（状態遷移図）

あなたが設計した課金制1対1音声通話アプリの要件に最適化した形でまとめます。
...
| **CONNECTING** | SFUのProducer/Consumer作成中 |
| **TALKING** | 通話中（RTP alive） |
| **ENDING** | RTP停止 or 切断判定後の後処理 |
| **ENDED** | 完全終了 |
...
## 5. **CONNECTING（SFU準備・音声未接続）**

行うこと：

- RTC-01 capabilities取得
- RTC-02 send/recv transport作成
- RTC-03 DTLS connect
- RTC-04 Producer作成（音声送信）
- RTC-05 Consumer作成（音声受信）
...
- 課金停止
- 通話履歴を確定
- RTC-07 Cleanup API（Transport/Producer/Consumer破棄）
...
| 項目 | P2P | SFU版 |
| --- | --- | --- |
| 通話開始の確実性 | 低い（自己申告） | **100%（RTP検知）** |
| 異常切断検知 | ほぼ不可能 | **RTP停止で確実に判定** |
| 課金の正確性 | 誤差あり | **秒単位で正確** |
